...
//  SS_Label_Info_2.1.4  #define genders and max age
  ...
  ivector ALK_OK(1,nseas*N_subseas);
...
//  example call to a routine in GLOBALS to do some of above
//            data_timing=get_data_timing(read_seas_mo, N_subseas, Svy_data(i)(1,3), seasdur, subseasdur_delta, azero_seas);
        ...
        if(y>=styr && y<=endyr)
        {
         ...
         j=disc_N_fleet(f);  //  index number for data that are in date range
          ...
          else  //  reading month.fraction
          {
            ...
            data_timing=(temp1-azero_seas(s))/seasdur(s);  //  remainder converted to fraction of season
          }
          t=styr+(y-styr)*nseas+s-1;
          disc_time_t(f,j)=t;
...
//    echoinput<<"settle and timing "<<settle<<" "<<Settle_timing(settle)<<endl;
   ...
   for (subseas=1;subseas<=N_subseas;subseas++)
   {
     ...
     curr_age(g,ALK_idx)=r_ages+azero_seas(s)-azero_G(g)+double(subseas-1)/double(N_subseas)*seasdur(s);
     calen_age(g,ALK_idx)=curr_age(g,ALK_idx)+azero_G(g);
     if(azero_G(g)>=azero_seas(s))
     {
       a=0;
       while(curr_age(g,ALK_idx,a)<0.0)
    ...
    if(do_migration>0)
    {
      ...
      if(use_morph(g)>0 && firstBseas(GP4(g))==Bseas(g))
      {
        ...
        for (subseas=1;subseas<=N_subseas;subseas++)
        {
          a=0;
          ALK_idx=(s-1)*N_subseas+subseas;
          while(curr_age(g,ALK_idx,a)<migr_firstage) {a++;}
...
//  SS_Label_Info_6.8 #Go thru biological calculations once, with do_once flag=1 to produce extra output to echoinput.sso
    ...
    ALK_OK=0;  //  vector to indicate if ALK needs recalculating
    do_once=1;
    niter=0;
    y=styr;
    yz=styr;
...
//   get mean size at the beginning and end of the season
  ...
  //  SS_Label_Info_16.2.3  #Set up Lmin and Lmax
          ...
          if(Grow_type!=2)  //  not Richards, set-up Linf
          {
            if(AFIX2==999)
  ...
  //  SS_Label_Info_16.2.4.1.1  #if y=styr, get size-at-age in first subseason of first season of this first year
        if(do_once==1 && g==1) echoinput<<y<<" "<<s<<" growth gp,g: "<<gp<<" "<<g<<" Lmin: "<<Lmin(gp)<<" "<<L_inf(gp)<<" "<<-VBK(gp)<<" settleage "<<Settle_age(settle)<<endl;
                switch(Grow_type)
                {
                  default:
  //  SS_Label_info_16.2.4.1.2  #for non-Richards
                  {
                    Ave_Size(styr,1,g,0)=L_inf(gp) + (Lmin(gp)-L_inf(gp))*mfexp(VBK(gp,0)*VBK_seas(0)*(curr_age(g,1,0)-AFIX));
...
//                      }  //  end subseason
  ...
  //  SS_Label_Info_16.2.4.2.1.2  #after age loop, if(s=nseas) get weighted average for size_at_maxage from carryover fish and fish newly moving into this age
                  ...
                  case 2:   //  Richards
                  {
                    ...
                    for (a=0;a<=nages;a++)
                    {
  //  SS_Label_Info_16.2.4.2.2  #von Bert growth with Richards coefficient, loop ages
  //  SS_Label_Info_16.2.4.2.2.1  #NOTE - not updated to use subseas protocols
  //  SS_Label_Info_16.2.4.2.2.2  #calc midseas size-at-age from begin season size-at-age, accounting for transition from linear to von Bert as necessary
                      if(lin_grow(g,ALK_idx,a)>=0.0)  // in linear phase for begin seas
                      {
                        Ave_Size(t,1,g,a) = len_bins(1)+lin_grow(g,ALK_idx,a)*(Cohort_Lmin(gp,y,a)-len_bins(1));  // begin of season
                      }
                      if(lin_grow(g,ALK_idx_mid,a)>=0.0)  // in linear phase for midseas
                      {
                        Ave_Size(t,mid_subseas,g,a) = len_bins(1)+lin_grow(g,ALK_idx_mid,a)*(Cohort_Lmin(gp,y,a)-len_bins(1));  // mid season
                      }
  
                      if(lin_grow(g,ALK_idx,a)==-1.0)  // first time point beyond AFIX
                      {
                        temp=pow(Cohort_Lmin(gp,y,a),Richards(gp));
                        temp1=temp+(temp-LinfR)*(mfexp(VBK(gp,nages)*(curr_age(g,ALK_idx,a)-AFIX)*VBK_seas(s))-1.0)*Cohort_Growth(y,a);
                        Ave_Size(t,1,g,a) = pow(temp1,inv_Richards);
                      }
  
                      if(lin_grow(g,ALK_idx_mid,a)<0.0)  //  so doing growth curve by mid of this seas, so project with growth to begin of next
                      {
                        if(lin_grow(g,ALK_idx_mid,a)==-1.0)  // first time point beyond AFIX
                        {
                          temp=pow(Cohort_Lmin(gp,y,a),Richards(gp));
                          temp1=temp+(temp-LinfR)*(mfexp(VBK(gp,nages)*(curr_age(g,ALK_idx_mid,a)-AFIX)*VBK_seas(s))-1.0)*Cohort_Growth(y,a);
                          Ave_Size(t,mid_subseas,g,a) = pow(temp1,inv_Richards);
                        }
                        else  //  calc midseas size starting from size at beginning of this season
                        {
                          ...
                          Ave_Size(t,mid_subseas,g,a) = pow((temp+growhalf*(t2)*Cohort_Growth(y,a)),inv_Richards);
                        }
  
  //  SS_Label_Info_16.2.4.2.2.3  #calc size at end of the season, which will be size at begin of next season using current seasons growth parms
                      if(a<nages) {k2=a+add_age;} else {k2=a;}  // where add_age =1 if s=nseas, else 0
                        temp=pow(Ave_Size(t,mid_subseas,g,a),Richards(gp));
                        t2=temp-LinfR;  //  remaining growth potential
                        if(time_vary_MG(y,2)>0 && t2>-1.)
                        {
                          join1=1.0/(1.0+mfexp(-(50.*t2/(1.0+fabs(t2)))));  //  note the logit transform is not perfect, so growth near Linf will not be exactly same as with native growth function
                        ...
                        if((a<nages || s<nseas)) {Ave_Size(t+1,1,g,k2) = temp1;}
                      }
                    }  // done age loop
  //  SS_Label_Info_16.2.4.2.2.4  #after age loop, if(s=nseas) get weighted average for size_at_maxage from carryover fish and fish newly moving into this age
                    if(s==nseas)
                    {
                      temp=( (natage(t,1,g,nages-1)+0.01)*Ave_Size(t+1,1,g,nages) + (natage(t,1,g,nages)+0.01)*temp1) / (natage(t,1,g,nages-1)+natage(t,1,g,nages)+0.02);
                      Ave_Size(t+1,1,g,nages)=temp;
...
FUNCTION void get_initial_conditions()
  ...
  {
    ALK_OK.initialize();  //  to indicate that all ALKs need calculation
    get_growth2();   // initial year;  do growth before natmort in case lorenzen M is used
  }
  if(do_once==1) cout<<" growth OK"<<endl;
...
FUNCTION void get_time_series()
  ...
  //  SS_Label_Info_24.1.1 #Update the time varying biology factors if necessary
      ...
      if(time_vary_MG(y,2)>0)  
        {
          ALK_OK=0;  // indicate that all ALKs will need re-estimation
          get_growth2();  // years after styr;  do growth before natmort in case lorensen M is used
        }
                ...
                if(tune_F<F_Tune)
                {
                  ...
                  if (catch_seas_area(t,p,f)==1)
                  {
                    ...
                    if(use_morph(g)>0)
                    {
                      if(catchunits(f)==1)
                      {
                        interim_tot_catch+=Hrate(f,t)*elem_prod(natage(t,p,g),sel_al_2(s,g,f))*Zrate2(g);  // biomass basis
                      }
                      else
                      {
                        interim_tot_catch+=Hrate(f,t)*elem_prod(natage(t,p,g),sel_al_4(s,g,f))*Zrate2(g);  //  numbers basis
                      }
                    }  //close gmorph loop
                  }  // close fishery
                  ...
                  if(catch_seas_area(t,p,f)==1)
                  {
                    ...
                    if(use_morph(g)>0)
                    {
                      if(catchunits(f)==1)
                      {
                        vbio+=elem_prod(natage(t,p,g),sel_al_2(s,g,f)) *Zrate2(g);
                      }
                      else
                      {
 ...
 */
          ...
          if(docheckup==1)
          {
            echoinput<<" Morph "<<g<<endl<<" natM:"<<endl<<natM<<endl;
            if(F_Method>1) echoinput<<"Z-at-age "<<Z_rate(t,p,g)(0,min(6,nages))<<endl;
            echoinput<<" survivors "<<natage(t+1,p,g)(0,min(6,nages))<<endl;
          }
//      echoinput<<y<<" "<<s<<" "<<g<<" Z "<<Z_rate(t,p,g)<<endl;
//      echoinput<<y<<" "<<s<<" "<<g<<" survivors to next season "<<natage(t+1,p,g)<<endl;
        } // close gmorph loop
  }
         ...
         for (p=1;p<=pop;p++)
         {
           ...
           if(Fishon==1)
           {
             if(F_Method>=2)
             {
               ...
               if (fleet_area(f)==p && Hrate(f,t)>0.0)
               {
                 equ_catch_fleet(2,s,f)+=Hrate(f,t)*elem_prod(equ_numbers(s,p,g)(0,nages),deadfish_B(s,g,f))*Zrate2(g);      // dead catch bio
                 equ_catch_fleet(5,s,f)+=Hrate(f,t)*elem_prod(equ_numbers(s,p,g)(0,nages),deadfish(s,g,f))*Zrate2(g);      // deadfish catch numbers
                 equ_catch_fleet(3,s,f)+=Hrate(f,t)*elem_prod(equ_numbers(s,p,g)(0,nages),sel_al_2(s,g,f))*Zrate2(g);      // retained catch bio
                   equ_catage(s,f,g)=elem_prod(elem_prod(equ_numbers(s,p,g)(0,nages),deadfish(s,g,f)) , Zrate2(g));
                   equ_catch_fleet(1,s,f)+=Hrate(f,t)*elem_prod(equ_numbers(s,p,g)(0,nages),sel_al_1(s,g,f))*Zrate2(g);      // encountered catch bio
                   equ_catch_fleet(4,s,f)+=Hrate(f,t)*elem_prod(equ_numbers(s,p,g)(0,nages),sel_al_3(s,g,f))*Zrate2(g);      // encountered catch bio
                   equ_catch_fleet(6,s,f)+=Hrate(f,t)*elem_prod(equ_numbers(s,p,g)(0,nages),sel_al_4(s,g,f))*Zrate2(g);      // retained catch numbers
               }
             }
             else  // F_method=1
             {
...
FUNCTION void Make_AgeLength_Key(const int s, const int subseas)
  ...
  int gstart;
   ...
   if(ALK_OK(ALK_idx)==0) //  so need to calculate
   {
      ...
      for (settle=1;settle<=N_settle;settle++)
      {
        ...
        if(recr_dist_pattern(GPat,settle,0)>0)
        {
          for (gp2=1;gp2<=N_platoon;gp2++)      // loop the platoons
          {
            ...
            {
              ALK(ALK_idx,g)=calc_ALK_log(len_bins,use_Ave_Size_W,use_SD_Size);
            }
...
FUNCTION dvar_matrix calc_ALK_log(const dvector &len_bins, const dvar_vector &mean_len_at_age, const dvar_vector &sd_len_at_age)
 ...
 //SS_Label_FUNCTION_31.3 #Calculate the ALK with lognormal error, called when Grow_logN==1
  ...
  for (a = 0; a <= nages; a++)
  {
    ...
    for (z = 2; z <= nlength; z++) 
    { 
      len_dev = (log(len_bins(z)) - temp) / (sd_len_at_age(a));
      AL(z) = cumd_norm (len_dev);
    } // end length loop
    ALK_w(a) = first_difference(AL);
  }   // end age loop
...
//      if(time_vary_sel(yz,f+Nfleet)>0 || time_vary_sel(yz,f)>0 || time_vary_MG(yz,2)>0 || time_vary_MG(yz,3)>0)
      if(time_vary_makefishsel(yf,f)>0 || save_for_report>0)
      {
        ...
        if(seltype(f,2)>=2)
        {
          ...
          if(WTage_rd==0)
          {deadfish_B(s,g,f)=elem_prod(sel_a(yf,f,gg),(ALK_w * elem_prod(discmort2(yf,f,gg),wt_len(s,GP(g))) ));} // selected dead by weight
          else
 ...
 */
  ...
  if(N_MGparm_dev>0)
   ...
   for (settle=1;settle<=N_settle;settle++)
   {
      ...
      Settle_age(settle)<<" "<<Settle_timing(settle)<<" "<<recr_dist(gp,settle,p)<<endl;
   }
...
//                                             SS_Label_340
  ...
  SS2out<<"Fleet Yr.frac Gender Mkt Nsamp effN Like Period Use"<<endl;
   ...
   for (i=1;i<=Nobs_l(f);i++)
   {
     ...
     ALK_time=Len_time_ALK(f,i);
      ...
      if(header_l(f,i,3)>0)
      {
       ...
       SS2out<<f<<" "<<data_time(ALK_time,f,3)<<" "<<gen_l(f,i)<<" "<<mkt_l(f,i)<<" "<<nsamp_l(f,i)<<" "<<neff_l(f,i)<<" "<<
  ...
  rmse = 0.0;  n_rmse = 0.0; mean_CV=0.0;  Hrmse=0.0; Rrmse=0.0;
   ...
   for(i=1;i<=Nobs_a(f);i++)
     ...
     if(nsamp_a(f,i)>0 && header_a(f,i,3)>0)
      ...
      SS2out<<f<<" "<<header_a(f,i,1)<<" "<<abs(header_a(f,i,2))<<" "<<gen_a(f,i)<<" "<<mkt_a(f,i)<<" "<<ageerr_type_a(f,i)<<" "<<Lbin_lo(f,i)<<" "<<Lbin_hi(f,i)<<" "<<nsamp_a(f,i)<<" "<<neff_a(f,i)<<" "<<
...
//  make age-length key if needed
    ALK_idx=(s-1)*N_subseas+subseas;
    ALK_time=(y-styr)*nseas*N_subseas+(s-1)*N_subseas+subseas;
    if(ALK_OK(ALK_idx)==0)  //  check to see if ALK has already been updated for this subseason
    {
      if(need_ALK(ALK_time)>0)  //  this is related to the data
      {
        get_growth3(s, subseas);
        Make_AgeLength_Key(s, subseas);
      }
...
//  make selected age-length sample for this fleet and with this timing
        {
          ...
          if(use_morph(g)>0)
          {
            ...
            {tempvec_a=elem_prod(natage(t,p,g),elem_prod(Zrate2(g),sel_a(y,f,gg)));}
